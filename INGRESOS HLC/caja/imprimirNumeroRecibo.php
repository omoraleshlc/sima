...
$sSQL311a= "Select  * From cargosCuentaPaciente WHERE keyCAP='".$_GET['keyCAP']."' ";
$result311a=mysql_db_query($basedatos,$sSQL311a);
$myrow311a = mysql_fetch_array($result311a);


if($myrow311a['abonosCxC']=='si'){
    $myrow311['seguro']=$myrow311a['clientePrincipal'];
}
...
$pdf->SetXY(2,29);
if($myrow311['seguro']){
$sSQL1= "Select nomCliente From clientes WHERE entidad='".$_GET['entidad']."' AND numCliente = '".$myrow311['seguro']."' ";
...
$pdf->Cell(0,0,'Cajero: '.$myrow311a['usuario'],0,0,L);

//referencia
if($myrow311['folioVenta']!=NULL){
$pdf->SetXY(2,45);
...
$pdf->SetXY(2,25);
$pdf->Cell(0,0,'Recibo: '.$myrow311a['numRecibo'],0,0,R);
...
$fecha1=date("d/m/Y");
$pdf->SetY(30);
$pdf->Cell(0,0,'Fecha: '.cambia_a_normal($myrow311a['fecha1']),0,0,R);

//Citas
$pdf->SetY(34);
$pdf->Cell(0,0,'Hora Cargo: '.$myrow311a['hora1'],0,0,R);



$pdf->SetY(38);
$sSQL1y= "Select * From catCajas WHERE entidad='".$myrow311['entidad']."' AND keyCatC = '".$myrow311a['codigoCaja']."' ";
...
//Status
$sSQL1yalm= "Select descripcion From almacenes WHERE entidad='".$myrow311['entidad']."' AND almacen = '".$myrow311['almacen']."' ";
$result1yalm=mysql_db_query($basedatos,$sSQL1yalm);
$myrow1yalm = mysql_fetch_array($result1yalm);
$pdf->SetY(42);
$pdf->Cell(0,0,'P Venta: '.$myrow1yalm['descripcion'],0,0,R);



//Usuario
if($myrow311['autoriza']!=NULL){
$pdf->SetY(45);
...
//cantidad
$pdf->SetX('2');
$pdf->Cell(0,0,round($myrow['cantidad'],3),0,0,L);
...
$pdf->SetY($Y+10);
$pdf->Cell(0,0,$descripcionExento.": "."-$ ".number_format($totalDescuento[0],2),0,0,R);
}

if($totalCortesia[0]>0){
$pdf->SetY($Y+10);
$pdf->Cell(0,0,$descripcionExento.": "."-$ ".number_format($totalCortesia[0],2),0,0,R);
}

if($totalBeneficencia[0]>0){
$pdf->SetY($Y+10);
$pdf->Cell(0,0,$descripcionExento.": "."-$ ".number_format($totalBeneficencia[0],2),0,0,R);
}


$pdf->SetY($Y+15);
$pdf->Cell(0,0,"Total: "."$ ".number_format($TOTAL-$totalDescuento[0]-$totalBeneficencia[0]-$totalCortesia[0],2),0,0,R);
