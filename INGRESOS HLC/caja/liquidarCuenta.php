...
$nCuenta1=$myrow3['nCuenta'];
//***************aplicar pago**********************
if($myrow3['tipoPaciente']=='externo' and $_GET['aplicarPago'] ){
$q = "UPDATE cargosCuentaPaciente set 
status='pagado'


WHERE numeroE = '".$numeroE1."' ";
mysql_db_query($basedatos,$q);
echo mysql_error();

} else {
if($_GET['aplicarPago'] AND $_GET['campoDespliega']){
...
//mysql_db_query($basedatos,$q);
echo mysql_error();
$recibida=$_GET['cantidadRecibida'];
...
numeroE,nCuenta,status,usuario,fecha1,cantidadRecibida,tipoTransaccion,codProcedimiento,hora1,dia,numPoliza,
ctoCosto,auxiliar,ejercicio,statusDeposito,cantidad,naturaleza
) values ('".$numeroE1."','".$nCuenta1."','pagado',
'".$usuario."','".$fecha1."','".$recibida."','".$_GET['tipoTransaccion']."','".$hora1."','".$hora1."','".$dia."',
'".$poliza."','".$ctoCosto."','".$auxiliar."','".$ID_EJERCICIOM."','pagado','1','".$naturaleza."'
...
?>  <?PHP if($myrow3['tipoPaciente']=='interno'){ ?>
...
$Tcargos=$myrow17['sumaCargos']+$myrow171['CantidadRecibida'];
		//echo "$".number_format($iva,2);
?>
          ...
          <tr bgcolor="#FFFFFF">
            ...
            <td class="Estilo24"><?php 
			if($Tcargos){
			echo "$".number_format($Tcargos-$Tabonos,2);
			} else {
			echo "---";
			}
			?></td>
          ...
          <tr bgcolor="#FFFFFF">
			...
			echo $Tcargos;
			if($Tcargos<0){
			echo substr($Tcargos-$Tabonos,1);
			} else {
			echo $Tcargos-$Tabonos;
			}
			} else if($_GET['totalPagar']){ 
			
			if($_GET['totalPagar']<0){
			echo substr($_GET['totalPagar'],1);
			} else {
			echo $_GET['totalPagar'];
			}
				}
			 
			 ?>" autocomplete="off"/>
			<input name="TOTAL" type="hidden" class="style7" id="cantidadRecibida" value="<?php echo $TOTAL;?>"/>	<input name="nT" type="hidden" class="style7" id="nT" value="<?php echo $_GET['nT'];?>"/></td>
          </tr>
      </table></td>
    </tr>
  </table>
