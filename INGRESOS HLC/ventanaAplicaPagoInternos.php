...
<select name="tipoPago" class="style20" id="tipoPago" onChange="javascript:form.submit();" <?php if($_GET['tipoCliente']!=NULL) //echo 'disabled=""';?>>
            ...
            <?php }else if($_GET['abono']){ //AQUI SON AVONOS ?>
<select name="tipoPago" class="style20" id="tipoPago" onChange="javascript:form.submit();" <?php if($_GET['tipoCliente']!=NULL) //echo 'disabled=""';?>>
...
<option value="">Escoje el Tipo de Transacci�n</option>
   ...
   Calendar.setup({ 
    inputField     :    "campo_fecha",     // id del campo de texto 
     ...
     button     :    "lanzador"     // el id del bot�n que lanzar� el calendario 
            ...
            <tr bgcolor="#FFFFFF">
              <td class="Estilo29">Compa&ntilde;&iacute;a</td>
              <td class="Estilo24"><span class="style16">
			  
                <?php 
		$company=$cargosAseguradoraCC->cargosAseguradoraCC($basedatos,$usuario,$_GET['nCuenta']);
		echo "$".number_format($cargosAseguradoraCC->cargosAseguradoraCC($basedatos,$usuario,$_GET['nCuenta']),2)." ";?>
              </span></td>
            </tr>
   
            <tr bgcolor="#FFFFFF">
              <td class="Estilo29">Particular</td>
              <td class="Estilo24"><span class="style16">
                <?php 
		echo '$'.number_format($cargosParticularesCC->cargosParticularesCC($basedatos,$usuario,$_GET['nCuenta']),2);
...
if($seguro and $myrow31ab['pagoEfectivo']==''){ 
				...
				} else { //no es una nota de devoluci�n
		...
		}//termina validacion de devoluci�n	
				...
				if(($valor or $myrow3['tipoPaciente']=='interno') or $_GET['devolucion']=='si'){
...
if($_GET['devolucion']=='si' or ($_GET['abono']=='si'  and $_GET['tipoPago']) or (($valor>0  and $_GET['tipoPago'] and $transaccion) or $_GET['paquete']=='si')) {  ?>
...
if(
($_GET['devolucion']=='' or !$_GET['paquete'] and $_GET['tipoPago']!='Efectivo' and $_GET['tipoPago']!='Nomina' and  $_GET['tipoPago']!='Cheque' and $_GET['tipoPago']!='Tarjeta de Credito') or ($myrow3['tipoPaciente']=='interno' AND $_GET['tipoTransaccion']) ){ 
//echo 'readonly=""';
}
}
...
<label>
  ...
  <span class="Estilo24">
     ...
     <?php if($_GET['devolucion']=='si' or $_GET['abono']=='si' or ($valor>0 AND $_GET['tipoPago'] and $transaccion) or $_GET['paquete']=='si'){ ?>
