...
$sSQL1y = mysql_real_escape_string($sSQL1y); 
$sSQL1y= "Select * From clientes WHERE entidad='".$_GET['entidad']."' AND numCliente = '".$_GET['seguro']."' ";
...
$pdf->Cell(0,0,'Cliente: '.$myrow1y['nomCliente'],0,0,L);
}
...
$pdf->SetY(38);
$sSQL1y = mysql_real_escape_string($sSQL1y); 
$sSQL1y= "Select * From catCajas WHERE entidad='".$_GET['entidad']."' AND keyCatC = '".$_GET['numMovimiento']."' ";
$result1y=mysql_db_query($basedatos,$sSQL1y);
$myrow1y = mysql_fetch_array($result1y);
...
$sSQL4 = mysql_real_escape_string($sSQL4); 
...
if($myrow['tipoPago']=='Transferencia Electronica'){
$descripcion=$myrow['tipoPago'].', '.$myrow['bancoTransferencia'].', # '.$myrow['numeroTransferencia'];
}else if($myrow['tipoPago']=='Tarjeta de Credito'){
$descripcion=$myrow['tipoPago'].', '.$myrow['bancoTC'].', CodAut: '.$myrow['codigoAutorizacion'].', U.Digitos: '.$myrow['ultimosDigitos'];
}else if($myrow['tipoPago']=='Cheque'){
$descripcion=$myrow['tipoPago'].', '.$myrow['bancoCheque'].', #: '.$myrow['numeroCheque'];
}else{
    if($myrow['statusDevolucion']=='si'){
$descripcion=$myrow['descripcionArticulo'];
    }else{
