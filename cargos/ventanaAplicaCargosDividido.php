...
if(!$_GET['secure'] and $myrow3['securo']){
$secure=$myrow3['seguro'];
} else if($_GET['secure']){
$secure=$_GET['secure'];
}
...
$_GET['tipoPago']='Cuentas por Cobrar';
}
//***************aqui modificado
...
$sSQL455= "Select clientePrincipal from clientes where entidad='".$entidad."' and numCliente='".$secure."'";
...
if($myrow3['status']=='devolucion'){
$q4 = "UPDATE clientesInternos set 
statusCuenta='cargado'
...
if($_GET['aplicarPago'] AND $_GET['cantidadRecibida'] AND $_GET['tt']){
...
$q4 = "UPDATE clientesInternos set dividirCuentas='".$_POST['dividirCuentas']."',
numRecibo='".$RECIBO."',numCorte='".$numCorte."',codigoCaja='".$myrowC['keyCatC']."',
statusCuenta='abierta',status='activa',statusDeposito='pagado',usuario='".$usuario."',fecha1='".$fecha1."'
WHERE keyClientesInternos='".$numeroCuenta."'";
mysql_db_query($basedatos,$q4);
echo mysql_error();
}
...
ultimosDigitos,
telefono,bancoTC,bancoTransferencia,numeroTransferencia,numeroCheque,bancoCheque,precioVenta,statusDevolucion,random,codigoAutorizacion
...
'".$secure."','trasladado','particular','".$myrow3['tipoPaciente']."',
...
'".$_GET['ultimosDigitos']."','".$_GET['telefono']."','".$_GET['bancoTC']."','".$_GET['bancoTransferencia']."','".$_GET['numeroTransferencia']."','".$_GET['numeroCheque']."','".$_POST['bancoCheque']."','".$_GET['cantidadRecibida']."','".$_GET['devolucion']."','".$_GET['random']."','".$_GET['codigoAutTC']."')";
...
numeroE,nCuenta,status,usuario,fecha1,dia,cantidad,tipoTransaccion,codProcedimiento,hora1,
naturaleza,ejercicio,statusDeposito,almacen,usuarioTraslado,seguro,
statusTraslado,tipoCliente,tipoPaciente,cantidadParticular,cantidadAseguradora,numCorte,entidad,tipoCobro,statusAuditoria,tipoPago,almacenDestino,keyClientesInternos,statusFactura,statusCargo,statusImpresion,numRecibo,folioVenta,codigoCaja,statusCaja,telefono,clientePrincipal,precioVenta,statusDevolucion,bancoCheque,numeroCheque,random,codigoAutorizacion,ultimosDigitos,bancoTC) 
values 
...
'".$secure."','".$statusTraslado."','".$_GET['tipoCliente']."','".$myrow3['tipoPaciente']."',
'".$cantidadParticular."','".$cantidadAseguradora."','".$numCorte."','".$entidad."','".$_GET['tipoPago']."','standby'
,'".$_GET['tipoPago']."','".$ALMACEN."','".$numeroCuenta."','standby','cargado','standby','".$RECIBO."','".$myrow3['folioVenta']."','".$myrowC['keyCatC']."','pagado','".$_GET['telefono']."','".$myrow455['clientePrincipal']."','".$_GET['cantidadRecibida']."','".$_GET['devolucion']."','".$_GET['bancoCheque']."','".$_GET['numeroCheque']."','".$_GET['random']."','".$_GET['codigoAutTC']."','".$_GET['ultimosDigitos']."','".$_GET['bancoTC']."')";
mysql_db_query($basedatos,$agrega);
echo mysql_error();
...
values 
('".$numeroE."','".$nCuenta."','transaccion',
'".$usuario."','".$fecha1."','".$dia."','1','".$_GET['tt']."','".$hora1."',
...
if(!$_GET['tipoPago'] and $_GET['tipoVenta']=='externo' and !$_GET['abono']){ 
...
?>
    ...
    <tr bgcolor="#990033">
      <td bgcolor="#FFFFFF"><table width="99%" height="394" border="0" cellpadding="4" cellspacing="0">
          <tr bgcolor="#FFFFFF">
            <td width="25%" class="style16">Tipo Pago/Cr&eacute;dito </td>
            <td width="75%" class="style16">
		
		<?php if($_GET['tipoVenta']=='interno' and !$_GET['abono']){ ?>
        <?php 
		if($_GET['tipoTransaccion']=='particular' || $_GET['tipoTransaccion']=='otros' || $_GET['tipoTransaccion']=='coaseguro'){ ?>
		...
		<select name="tipoPago" class="style20" id="tipoPago" onChange="javascript:form.submit();" <?php if($_GET['tipoCliente']!=NULL) //echo 'disabled=""';?>>
             <option value="">Escoje...</option>
				...
				<?php if($_GET['tipoPago']=='Cheque'){ ?>
                 ...
                 <?php if($myrow455['cargoNomina']=='si'){ ?>
                 <option 
		...
		<?php } else if($_GET['tipoTransaccion']=='aseguradora'){ ?>
        ...
        <?php } else {  //es un cliente ambulatorio o externo?>
...
<select name="tipoPago" class="style20" id="tipoPago" onChange="javascript:form.submit();" <?php if($_GET['tipoCliente']!=NULL) //echo 'disabled=""';?>>
            ...
            <?php } else if($myrow3['seguro']=='' or $myrow3['seguro']=='0' or $myrow3['paquete']=='si' or $_GET['paquete']=='si'){ ?>
         			<select name="tipoPago" class="style20" id="tipoPago" onChange="javascript:form.submit();" <?php if($_GET['tipoCliente']!=NULL) //echo 'disabled=""';?>>
			...
			<?php }else { 
			echo $_GET['tipoPago']='Cuentas por Cobrar';
			?>
...
$myrow3ac1 = mysql_fetch_array($result3ac1); 
		$_GET['transaccion']=$myrow3ac1['codigoTT'];
          ...
          <tr bgcolor="#FFFFFF">
           ...
           <tr bgcolor="#FFFFFF">
            ...
            <tr bgcolor="#FFFFFF">
              <td class="Estilo29">&nbsp;</td>
              <td class="Estilo24"><span class="negromid">
                <input name="secure" type="hidden" class="camposmid" id="secure"   readonly=""
		value="<?php if($_POST['seguro'] and !$_POST['nuevo']){ echo $_POST['seguro'];} else { echo "0";}?>" 
		onchange="javascript:this.form.submit();" />
              </span></td>
            </tr>
            <tr bgcolor="#FFFFFF">
              <td class="Estilo29">Aseguradora</td>
              <td class="Estilo24"><input name="nomSeguro" type="text" class="Estilo24" id="nomSeguro"
		value="<?php 
		...
		?>" size="80"/></td>
          ...
          <tr bgcolor="#FFFFFF">
            ...
            <td class="Estilo24"><span class="style16">
...
if($myrow31a['deposito']){//******************TRAE DEPOSITO
$valor=$myrow31a['deposito'];	
}else{
$valor=$cargosParticularesCC->cargosParticularesCC($basedatos,$usuario,$_GET['nCuenta']);
$valor='Escribe la Cantidad';
}//*****************************************
...
//pESTE SOLO ES para AlTa de PaCientEs
		...
		$valor=$cargosParticularesCC->cargosParticularesCC($basedatos,$usuario,$_GET['nCuenta']);
		} else if($_GET['tipoTransaccion']=='otros'){ 
		$valor=$otros->otros($basedatos,$usuario,$numeroE,$nCuenta);
...
if($seguro and $myrow31ab['pagoEfectivo']==''){ 
				...
				if($valor or $myrow3['tipoPaciente']=='interno'){
...
if(($_GET['abono']=='si'  and $_GET['tipoPago']) or (($valor>0  and $_GET['tipoPago'] and $transaccion) or $_GET['paquete']=='si')) {  ?>

               
               
               

<input name="cantidadRecibida" type="text" class="style15" id="cantidadRecibida" value="<?php echo round($valor,2);?>" autocomplete="off" 
<?php if((!$_GET['paquete'] and $_GET['tipoPago']!='Efectivo' and $_GET['tipoPago']!='Nomina' and  $_GET['tipoPago']!='Cheque' and $_GET['tipoPago']!='Tarjeta de Credito') or ($myrow3['tipoPaciente']=='interno' AND $_GET['tipoTransaccion']) and !$_GET['abono']){ 
//echo 'readonly=""';
}
...
<label>
  ...
  <span class="Estilo24">
      ...
      <?php if($_GET['abono']=='si' or ($valor>0 AND $_GET['tipoPago'] and $transaccion) or $_GET['paquete']=='si'){ ?>
  ...
  <script>
		new Autocomplete("nomSeguro", function() { 
			this.setValue = function( id ) {
				document.getElementsByName("secure")[0].value = id;
			}
