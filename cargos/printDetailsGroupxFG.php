...
$pdf->SetX('185');
$pdf->Cell(0,0,'$'.number_format($myrow7j['acumulado'],2),0,0,R);
$pdf->Ln(3); //salto de linea
}
}
}
...
";
  ...
  $sSQL7ud="SELECT SUM(cantidadAseguradora*cantidad) as acumulado,SUM(ivaAseguradora*cantidad) as sumaIVA
FROM
cargosCuentaPaciente
WHERE
...
";
  ...
  $result7ud=mysql_db_query($basedatos,$sSQL7ud);
  $myrow7ud = mysql_fetch_array($result7ud);
  
  $myrow7u['acumulado']-=$myrow7ud['acumulado'];
  $myrow7u['sumaIVA']-=$myrow7ud['sumaIVA'];
