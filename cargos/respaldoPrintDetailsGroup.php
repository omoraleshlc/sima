...
//*******************MOVIMIENTOS
 ...
 $sSQLt="SELECT SUM(precioVenta*cantidad) as totalCliente
  FROM
...
if($C){
 $sSQL7="SELECT 
 SUM(precioVenta*cantidad) as acumulado
  FROM
  cargosCuentaPaciente
  WHERE
entidad='".$_GET['entidad']."'
and
  ...
  $sSQL7d="SELECT SUM(precioVenta*cantidad) as acumulado
  FROM
  cargosCuentaPaciente
  WHERE
entidad='".$_GET['entidad']."'
...
$sSQL7jd="SELECT SUM(cantidadAseguradora*cantidad) as acumulado
FROM
cargosCuentaPaciente
WHERE
entidad='".$_GET['entidad']."'
...
$pdf->Ln(3); //salto de linea
$pdf->SetX(22);
$pdf->Cell(0,0,$myrow38e1['descripcion'].$myrow7['acumulado'],0,0,L);
$pdf->SetX('185');
$brake=1;
...
if($brake>0){
$pdf->Cell(0,0,'$'.number_format($subTotal,2),0,0,R);
}
...
//**************TIPO ACCION***************//
if($myrow7f1['coaseguro']>0){
$tipoAccion[0]='coaseguro';
}
//*****************************************//
//$pdf->Ln(3); //salto de linea
...
$pdf->SetFont('Arial','',10);
$pdf->SetXY(1,218);
$pdf->Cell(0,0,"$".number_format(($sumaSubTotal[0])-$coaseguro[0],2),0,0,R);
...
$TOTAL=(($sumaSubTotal[0]+$ivaCargos[0])-($abonos[0]+$ivaAbonos[0]))-$coaseguro[0];
