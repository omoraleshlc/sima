...
$sSQL311= "Select  * From facturasAplicadas WHERE numFactura='".$_GET['folioFactura']."'";
$result311=mysql_db_query($basedatos,$sSQL311);
$myrow311 = mysql_fetch_array($result311);


$sSQL311= "Select  * From clientesInternos WHERE folioVenta='".$myrow311['folioVenta']."'";
$result311=mysql_db_query($basedatos,$sSQL311);
$myrow311 = mysql_fetch_array($result311);
...
$sSQL455= "Select * from clientes where entidad='".$myrow311['entidad']."' and numCliente='".$myrow311['seguro']."'";
$result455=mysql_db_query($basedatos,$sSQL455);
$myrow455 = mysql_fetch_array($result455);



$sSQL455= "Select * from clientes where entidad='".$myrow311['entidad']."' and numCliente='".$myrow455['clientePrincipal']."'";
$result455=mysql_db_query($basedatos,$sSQL455);
$myrow455 = mysql_fetch_array($result455);
...
$myrow['precioVenta']=$myrow['cantidadParticular'];
...
//*******************MOVIMIENTOS
$sSQL7="SELECT SUM(cantidadParticular) as acumulado,SUM(ivaParticular) as sumaIVA
FROM
...
$pdf->Cell(0,0,'$'.number_format( $myrow7['acumulado'],2),0,0,L);

$pdf->SetX('185');
$pdf->Cell(0,0,'$'.number_format($myrow['precioVenta']*$myrow['cantidad'],2),0,0,R);

$ss[0]+=($myrow['precioVenta']*$myrow['cantidad']);
//cantidad
//$pdf->SetX('27');
...
$pdf->SetFont('Arial','',8);
$pdf->SetX(22);
$pdf->Cell(0,0,'Folio(s): ',0,0,L);
  $result71a=mysql_db_query($basedatos,$sSQL71a);
  while($myrow71a = mysql_fetch_array($result71a)){
  
  $x=33; 
   $pdf->SetX($x+$ir);
$pdf->Cell(0,0,$myrow71a['folioVenta'],0,0,L);
$ir+=10;
  }
...
$sql4 = "UPDATE cargosFacturados set 

statusImpresion='cargado'

WHERE numFactura='".$_GET['folioFactura']."' and
keyClientesInternos='".$_GET['keyClientesInternos']."' 
and
status='facturado' and statusImpresion='standby'
";

//mysql_db_query($basedatos,$sql4);
//echo mysql_error();


$TOTAL=$ss[0]+$sumaIVA[0];
$TOTAL= round($TOTAL,2);
...
$pdf->SetFont('Arial','',10);
$pdf->SetXY(1,224);
$pdf->Cell(0,0,"$".number_format($ss[0],2),0,0,R);


$pdf->SetXY(1,233);
$pdf->Cell(0,0,"$".number_format($sumaIVA[0],2),0,0,R);

$pdf->SetXY(1,242);
$sumaTotales=$importe2[0]+$sumaIVA[0];
$pdf->Cell(0,0,"$".number_format($ss[0]+$sumaIVA[0],2),0,0,R);
