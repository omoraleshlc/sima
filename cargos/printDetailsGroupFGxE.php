...
$fecha1=cambia_a_normal($_GET['fechaImpresion']);
}else{
$fecha1=date("d    m    Y");
}
$pdf->SetXY(50,50);
...
$sSQL3abe= "Select gpoProducto From facturasAplicadas WHERE folioVenta = '".$_GET['folioVenta']."'  and extension='".$_GET['extension']."'";
$result3abe=mysql_db_query($basedatos,$sSQL3abe);
$myrow3abe = mysql_fetch_array($result3abe);


if(!$myrow3abe['gpoProducto']){
$sSQL3abe1= "Select * From facturasAplicadas WHERE folioVenta = '".$_GET['folioVenta']."' and gpoProducto!=''";
$result3abe1=mysql_db_query($basedatos,$sSQL3abe1);
$myrow3abe1 = mysql_fetch_array($result3abe1);
}
...
$sSQL3ab= "Select porcentajeFacturacion From facturasAplicadas WHERE folioVenta = '".$_GET['folioVenta']."'  and extension='".$_POST['extension']."'";
...
$sSQL7d="SELECT SUM(precioVenta*cantidad) as acumulado,SUM(iva*cantidad) as sumaIVA
  FROM
  cargosCuentaPaciente
  WHERE
  folioVenta='".$_GET['folioVenta']."'
...
//*******************************************************************
 ...
 $sSQL38d= "
  ...
  FROM
  facturasAplicadas
  WHERE folioVenta='".$_GET['folioVenta']."' and extension='".$_GET['extension']."'";
  $result38d=mysql_db_query($basedatos,$sSQL38d);
  $myrow38d = mysql_fetch_array($result38d);
//*********************************************************************
// echo $myrow7['acumulado'].' '.$myrow38d['porcentajeFacturacion'].' --->'.$myrow7['acumulado']*$myrow38d['porcentajeFacturacion'].'<br>';
//el porcentaje facturacion cambia si ya esta el grupo asignado
if($myrow3abe1['gpoProducto']){
$myrow38d['porcentajeFacturacion']=0;
}
$myrow38d['porcentajeFacturacion']=0;


if($myrow7['acumulado']>0 && $myrow38d['porcentajeFacturacion']>0){ 
$importe2[0]+=$myrow7['acumulado']*$myrow38d['porcentajeFacturacion'];
$total=$myrow7['acumulado']*$myrow38d['porcentajeFacturacion'];
 $iva=$myrow7['sumaIVA']*$myrow38d['porcentajeFacturacion'];
 $ivaT[0]+=$myrow7['sumaIVA']*$myrow38d['porcentajeFacturacion'];
}else{
 $iva=$myrow7['sumaIVA'];
$total=$myrow7['acumulado'];
  $importe2[0]+=$myrow7['acumulado'];
  $ivaT[0]+=$myrow7['sumaIVA'];
}
...
if($myrow7d['acumulado']>0 && $myrow38d['porcentajeFacturacion']>0){ 

    $importe2D[0]+=$myrow7d['acumulado']*$myrow38d['porcentajeFacturacion'];
	$totalDev=$myrow7d['acumulado']*$myrow38d['porcentajeFacturacion'];
	 $ivaD=$myrow7d['sumaIVA']*$myrow38d['porcentajeFacturacion'];
	  $ivaTD[0]+=$myrow7d['sumaIVA']*$myrow38d['porcentajeFacturacion'];
	 
	}else{
	$totalDev=$myrow7d['acumulado'];
  	$importe2D[0]+=$myrow7d['acumulado'];
	 $ivaD=$myrow7d['sumaIVA'];
	 $ivaTD[0]+=$myrow7d['sumaIVA'];
}	
...
$s+=10;
$sSQL7j="SELECT SUM(precioVenta*cantidad) as acumulado
FROM
cargosCuentaPaciente
WHERE
folioVenta='".$myrow['folioVenta']."'
...
$pdf->Cell(0,0,'$'.number_format($total-$totalDev,2),0,0,R);
} else {
$pdf->Cell(0,0,'',0,0,R);
}
...
if($myrow38['tasaGP']=='0'){
$tasaCero[0]+=($total-$totalDev);
} else if($myrow38['tasaGP']>0){
$gravado=$myrow38['tasaGP'];
$tasaIVA[0]+=($total-$totalDev);
$sumaIVA[0]+=($iva-$ivaD);
} else if($myrow38['tasaGP']=='E'){
$tasaExento[0]+=($total-$totalDev);
}
//*********************************************************
...
folioVenta='".$_GET['folioVenta']."'
and
(naturaleza='A' and statusDevolucion!='si')
and
...
folioVenta='".$_GET['folioVenta']."'
and
(naturaleza='A' and statusDevoucion!='si')
and
tipoCliente='coaseguro'";
...
$sSQL7u="SELECT SUM(precioVenta*cantidad) as acumulado,SUM(ivaAseguradora*cantidad) as sumaIVA
FROM
cargosCuentaPaciente
WHERE
folioVenta='".$_GET['folioVenta']."'
...
$TOTAL=number_format(($importe2[0]+$ivaT[0])-($importe2D[0]+$ivaTD[0]),2);
...
$pdf->SetFont('Arial','',10);
$pdf->SetXY(1,224);
$pdf->Cell(0,0,"$".number_format($importe2[0]-$importe2D[0],2),0,0,R);

$pdf->SetXY(1,233);
$pdf->Cell(0,0,"$".number_format($ivaT[0]-$ivaTD[0],2),0,0,R);

$pdf->SetXY(1,242);

$pdf->Cell(0,0,"$".number_format(($importe2[0]+$ivaT[0])-($importe2D[0]+$ivaTD[0]),2),0,0,R);
