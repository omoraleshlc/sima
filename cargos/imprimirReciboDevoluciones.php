...
if($myrow311['nombreCliente']){
$pdf->Cell(0,0,'Paciente: '.$myrow311['nombreCliente'],0,0,L);
} else {
...
//************************************************CARGOS***************************************************
$sSQL317= "Select codigoTT,descripcion From catTTCaja WHERE banderaDevolucionCargo = 'si'";
$result317=mysql_db_query($basedatos,$sSQL317);
$myrow317 = mysql_fetch_array($result317);

$sSQL= "SELECT precioVenta,iva
FROM
...
$pdf->SetX('7');
$pdf->Cell(0,0,$myrow317['descripcion'],0,0,L);
$pdf->Cell(0,0,'$'.number_format($myrow['precioVenta'],2),0,0,R);
$cargo=$myrow['precioVenta'];
//**************************************************************************************************
...
//**********************************************ABONOS*****************************************
$sSQL341= "Select codigoTT,descripcion From catTTCaja WHERE banderaDevolucionAbono = 'si'";
$result341=mysql_db_query($basedatos,$sSQL341);
$myrow341 = mysql_fetch_array($result341);

$sSQL4= "SELECT precioVenta
FROM
cargosCuentaPaciente
WHERE 
folioVenta='".$myrow311['folioVenta']."'
and
tipoTransaccion='".$myrow341['codigoTT']."'
...
$pdf->SetX('7');
$pdf->SetFont('Arial','',13);
$pdf->Cell(0,0,$myrow341['descripcion'],0,0,L);
$pdf->Cell(0,0,'-$'.number_format($myrow4['precioVenta'],2),0,0,R);
//********************************************************************************************************
...
$pdf->SetX('7');
$pdf->Cell(0,0,'Folio de Devolución: '.$myrow311['folioDevolucion'],0,0,L);
//**************************************************************************************************
...
$TOTAL=$myrow['precioVenta']+$myrow['iva'];
...
$pdf->Cell(0,0,"Subtotal: "."$".number_format($cargo,2),0,0,R);




$pdf->SetY($Y+5);
$pdf->Cell(0,0,"Iva: "."$".number_format($myrow['iva'],2),0,0,R);
$pdf->SetY($Y+10);
$pdf->Cell(0,0,"Total: "."$".number_format($cargo+$myrow['iva'],2),0,0,R);
