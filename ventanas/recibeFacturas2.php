...
$ivaNC=($myrow['precioUnitario']*$myrow['cantidad'])*($myrow39e['tasaGP']*0.01);
}
//CIERRA TOTALES
...
?>
        ...
        <td bgcolor="<?php echo $color;?>" >
            <div align="center">
            <input type="checkbox" name="notaCredito[]" value="si" <?php if( $myrow['notaCredito']=='si'){echo 'checked=""';}?> >
            </div>
        </td>
...
<td><?php echo '$'.number_format($notaCredito[0],2);?></td>
</tr>
...
$tare= (float) noRound(($importe[0]+$myrow17a['gastos']+($iva[0]+$ivaGasto-($tivaOferta[0]+$tivaDescuento[0]+$myrow17a['ivaDescuentoPP'])))-($oferta[0]+$descuento[0]+$myrow17a['descuentoPP']),2);
echo '$'.$tare;
?>        
</td>
...
$total=$myrow17a['importe']-$tare;

if(($total>=-0.90 and $total<0 )or ($total>0 and $total<=0.1)){
    $total=(int) $total;
}else{
    $total=(float) $total;
}
