...
if($myrow39e['tasaGP']>0){
    echo  '<br>';
    echo '<span class="precio1"><blink>IVA</blink></span>';
...
<tr>
<td>Testing *Referencia Sistema</td>
...
$diferencia=($myrow17a['importe']+$myrow17a['iva'])-($subTotalCaptura-$rounding);
if($diferencia<=0 and $diferencia>=-0.90){
    $rounding=noRound($total*-1,2);
    echo '$'.number_format($subTotalCaptura-$rounding,2);  
    $t=$subTotalCaptura-$rounding;
}elseif($diferencia>=0 and $diferencia<=0.10){
    $rounding=noRound($total*1,2);
    echo '$'.number_format($subTotalCaptura+$rounding,2);   
    $t=$subTotalCaptura+$rounding;
}
...
$tt=($myrow17a['importe']+$myrow17a['iva']+$myrow17a['gastos'])-$t;
...
?>        
    ...
    <div align="center">
	  ...
	  if(( $diferencia>=0 and $diferencia<=0.5)|| ($diferencia>=-0.95 and $diferencia<=0)){ ?>
	  <br>
