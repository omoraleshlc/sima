...
$pdf->SetXY(2,5);
$pdf->Cell(0,0,'AGRUPADO DE POLIZA DE DIARIO, COMPRAS',0,0,M);
...
$pdf->SetXY(2,25);
$pdf->Cell(0,0,"REPOSICION X VENTA",0,0,M);
$pdf->SetX(1);
...
$sSQLa= "
SELECT almacenSolicitante FROM kardex
WHERE
...
$sSQLd= "SELECT sum(costo*cantidad) as cargos 
FROM kardex
WHERE
...
$sSQLh= "SELECT sum(costo*cantidad) as devoluciones 
FROM kardex
WHERE
...
$pdf->Cell(0,0,"AGRUPADO DE SOLICITUDES DIRECTAS AL CENTRO DE DISTRIBUCION",0,0,M);
$pdf->SetX(1); 
$pdf->Ln(5); //salto de linea
...
$pdf->Cell(0,0,'$'.number_format($myrow['costo'],2),0,0,M);
$debe[0]+=$myrow['costo'];
}
