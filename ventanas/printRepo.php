...
//$fecha1=$_GET['fecha'];
$pdf->SetY(10);
$pdf->Cell(0,0,'Solicitud: '.$_GET['nOrden'],0,0,R);



$pdf->SetXY(2,10);
$pdf->Cell(0,0,"SOLICITUD DIRECTA DEL PACIENTE",0,0,M);
$pdf->SetX(1); 
...
almacen='".$myrow['almacen']."'
  ";
  $result3acd=mysql_db_query($basedatos,$sSQL3acd);
  $myrow3acd = mysql_fetch_array($result3acd);
...
//
//
$diff=$surtido-$solicitado;
if($solicitado>0 and $diff==0){
    $stat='Cargado';
}else{
    $stat='Standby';
}
