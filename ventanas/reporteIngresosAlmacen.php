...
if($myrow['naturaleza'] == "C"){
    $sumaEfectivo[0] += ($myrow['cantidadParticular']*$myrow['cantidad'])+($myrow['ivaParticular']*$myrow['cantidad']);
    
    

$sumaCredito[0] += ($myrow['cantidadAseguradora']*$myrow['cantidad'])+($myrow['ivaAseguradora']*$myrow['cantidad']);

}
else if($myrow['naturaleza'] == "A"){
        $sumaEfectivoDevoluciones[0] += ($myrow['cantidadParticular']*$myrow['cantidad'])+($myrow['ivaParticular']*$myrow['cantidad']);
    
    

$sumaCreditoDevoluciones[0] += ($myrow['cantidadAseguradora']*$myrow['cantidad'])+($myrow['ivaAseguradora']*$myrow['cantidad']);
    
}

 $totalEfectivo1[0] = $sumaEfectivo[0] -  $sumaEfectivoDevoluciones[0];
 $totalCRedito1[0] = $sumaCredito[0] - $sumaCreditoDevoluciones[0];
...
$pdf->SetX('180');
$pdf->Cell(0,0,'$'.number_format($debe[0]-$haber[0],2),0,0,M);
...
$pdf->SetX('100');
$pdf->Cell(0,0,'Ventas en efectivo: ',0,0,M);

$pdf->SetX('130');

$pdf->Cell(0,0,' '.'$'.number_format($totalEfectivo1[0],2),0,0,M);



$pdf->Ln(4); //salto de linea */
$pdf->SetX('100');
$pdf->Cell(0,0,'Total ventas a credito: ',0,0,M);

$pdf->SetX('130');

$pdf->Cell(0,0,''.'$'.number_format($totalCRedito1[0],2),0,0,M);
...
$pdf->SetX('2');
$pdf->Cell(0,0,$myrow['numMovimiento'],0,0,M);
