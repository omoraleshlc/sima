...
$sSQL7na= "Select * from ALUMNOSINSCRITOS where MATRICULA='".$_GET['MATRICULA']."'";
...
$sSQLar= "Select * from clientesInternos where entidad='".$entidad."' and credencial='".$_GET['MATRICULA']."' 
and (fecha >='".$myrow7n['fechaInicial']."' and fecha<='".$myrow7n['fechaFinal']."' ) 
and
status='cerrada'
group by seguro
";
$resultar=mysql_db_query($basedatos,$sSQLar);
while($myrowar = mysql_fetch_array($resultar)){
    $seguro=$myrowar['seguro'];
$a+=1;
if($col){
$color = '#FFCCFF';
$col = "";
...
$sSQL= "Select * from clientesInternos where entidad='".$entidad."' and credencial='".$_GET['MATRICULA']."' 
and seguro='".$seguro."' 
and (fecha >='".$myrow7n['fechaInicial']."' and fecha<='".$myrow7n['fechaFinal']."' ) 
and
status='cerrada'
