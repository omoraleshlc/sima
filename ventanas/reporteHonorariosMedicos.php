...
//entidad='".$_GET['entidad']."'
//and
//almacen='".$myrow['almacenDestino']."' order by descripcion ASC ";
//
//
...
(fechaCierre>='".$_GET['fechaInicial']."' and fechaCierre<='".$_GET['fechaFinal']."')
and
almacenDestino='".trim($myrow['almacenDestino'])."'
and
gpoProducto='".$myrow6['codigoGP']."'
and
descripcionMedico!=''
...
entidad='".$_GET['entidad']."'
and
almacen='".$myrowg['almacenSolicitante']."' ";
...
//PX
//$pdf->Ln(3); //salto de linea
 $sSQL1b1a= "SELECT paciente FROM clientesInternos
WHERE
entidad='".$_GET['entidad']."'
and
folioVenta='".$myrowg['folioVenta']."' ";
...
$result1b1a=mysql_db_query($basedatos,$sSQL1b1a);
$myrow1b1a = mysql_fetch_array($result1b1a);
//********************
$as+=1;
$pdf->SetX('2');
$pdf->Cell(0,0,$as,0,0,M);
$pdf->Ln(3); //salto de linea
$pdf->SetX('15');
$pdf->Cell(0,0,$myrowg['folioVenta'],0,0,M);
$pdf->SetX('30');
$pdf->Cell(0,0,'Fecha Cierre: '.cambia_a_normal($myrow['fechaCierre']),0,0,M);
$pdf->Ln(3); //salto de linea
$pdf->SetX('15');
$pdf->Cell(0,0,$myrow1b1a['paciente'],0,0,M);
...
//****************************
$sSQLcMed="SELECT sum((precioVenta*cantidad)+(iva*cantidad)) as haber
...
descripcionMedico!=''
";
$resultcMed=mysql_db_query($basedatos,$sSQLcMed);
$myrowcMed = mysql_fetch_array($resultcMed);
...
descripcionMedico!=''
";
$resultdMed=mysql_db_query($basedatos,$sSQLdMed);
$myrowdMed = mysql_fetch_array($resultdMed);
//****************************************************************************
...
$pdf->Ln(4); //salto de linea *
$pdf->SetX('100');
$pdf->Cell(0,0,''.'$'.number_format($myrowdMed['debe'],2),0,0,M);

$pdf->SetX('130');
$pdf->Cell(0,0,' '.'$'.number_format($myrowcMed['haber'],2),0,0,M);
...
$pdf->Ln(4); //salto de linea */
$pdf->SetX('110');
$pdf->Cell(0,0,'TOTAL '.'$'.number_format($myrowdMed['debe']-$myrowcMed['haber'],2),0,0,M);
