...
$pdf->SetXY(2,10);
$pdf->SetFont('Arial','',8);
//$pdf->Cell(0,0,'Paciente: '.$_GET['paciente'],0,0,M);
$pdf->SetX(1); 
...
$pdf->SetXY(2,25);
$pdf->SetFont('Arial','',8);
//$pdf->Cell(0,0,'Medico: '.$_GET['medico'],0,0,L);
}
...
entidad='".$_GET['entidad']."'
and
    numSolicitudI='".$_GET['numSolicitud']."'
group by codigo
";
...
$sSQL2= "Select sum(cantidad) as c From articulosExistencias WHERE entidad='".$_GET['entidad']."' and numSolicitudI='".$_GET['numSolicitud']."'";
...
codigo='".$myrow['codigo']."'
  ...
  ";
  $result3ac2=mysql_db_query($basedatos,$sSQL3ac2);
  $myrow3ac2 = mysql_fetch_array($result3ac2);
...
$pdf->SetX('155');
if($myrow3ac['costo']>0){
$pdf->Cell(0,0,'$'.number_format($myrow3ac['costo'],2),0,0,M);
}

$pdf->SetX('180');
$pdf->Cell(0,0,$myrow2['c'],0,0,M);
