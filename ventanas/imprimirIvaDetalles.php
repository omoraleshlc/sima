...
$pdf->SetXY(10,5);
$pdf->Cell(0,0,'Fecha Impresion:  '.$fechaImpresion,0,0,R);
...
$sSQL1= "SELECT * FROM clientesInternos
WHERE
...
$debeI[0]+=($myrow['precioVenta']*$myrow['cantidad']);
$ivaDebe[0]+=($myrow['iva']*$myrow['cantidad']);
}else if($myrow['naturaleza']=='C' ){
$haberI[0]+=($myrow['precioVenta']*$myrow['cantidad']);
...
$pdf->Ln(4); //salto de linea */
$pdf->SetX('110');
$pdf->Cell(0,0,'SubTotal '.'$'.number_format($debeI[0]-$haberI[0],2),0,0,M);
$pdf->Ln(4); //salto de linea */



$pdf->SetX('110');
$pdf->Cell(0,0,'IVA '.'$'.number_format($ivaDebe[0]-$ivaHaber[0],2),0,0,M);
$pdf->Ln(4); //salto de linea */
$pdf->SetX('100');
$pdf->Cell(0,0,'* * * * * * * * * * * * * * * * * * * * * * * *',0,0,L);
...
$pdf->SetX('110');
$pdf->Cell(0,0,'TOTAL '.'$'.number_format(($debeI[0]+$ivaDebe[0])-($haberI[0]+$ivaHaber[0]),2),0,0,M);
$pdf->Ln(4); //salto de linea */
$pdf->SetX('100');
$pdf->Cell(0,0,'* * * * * * * * * * * * * * * * * * * * * * * *',0,0,L);
