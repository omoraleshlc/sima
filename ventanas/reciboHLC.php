...
$sSQL311en= "Select  * From entidades WHERE codigoEntidad='".$myrow311['entidad']."' ";
$result311en=mysql_db_query($basedatos,$sSQL311en);
$myrow311en = mysql_fetch_array($result311en);

$pdf->SetFont('Arial','B',13);

//establece el encabezado de la empresa
$pdf->SetXY(75,11);
$pdf->Cell(0,0,$myrow311en['descripcionEntidad'],0,0,M);

$pdf->SetFont('Arial','I',11);
$pdf->SetXY(54,15);
$pdf->Cell(0,0,$myrow311en['direccion'],0,0,M);
$pdf->SetXY(80,19);
$pdf->Cell(0,0,'CP: '.$myrow311en['cp'].', Tel. '.$myrow311en['telefono'],0,0,M);
$pdf->SetXY(82,22);
$pdf->Cell(0,0,$myrow311en['url'],0,0,M);
...
if($_GET['ticket']>0){
$pdf->SetXY(2,21);
$pdf->Cell(0,0,'Ticket: '.$_GET['ticket'],0,0,M);
}
...
if(($dev[0]+$ivaDev[0])>0){
$pdf->Ln(4); //salto de linea
$pdf->Cell(0,0,"Devoluciones: "."-$".number_format($dev[0]+$ivaDev[0],2),0,0,R);
$pdf->Ln(2); //salto de linea
$pdf->SetFont('Arial','',10);
$pdf->SetX(2);
...
if($_GET['ticket']>0){
$pdf->SetXY(2,$Y+4);
$pdf->Cell(0,0,'Ticket: '.$_GET['ticket'],0,0,L);


$pdf->SetXY(2,$Y+8);
$pdf->Cell(0,0,'http://www.hospitallacarlota.com',0,0,L);
}*/


if($_GET['ticket']>0){
    $td='El ticket para la facturacion electronica tiene una validez de 15 dias..';
}
...
//$pdf->Line(2,5, 199,5);
$pdf->SetFont('Arial','',10);
$pdf->Cell(0,0,'Direccion: http://www.hospitallacarlota.com',0,0,L);
$pdf->Cell(0,0,"Subtotal: "."$".number_format($cargos[0],2),0,0,R);
$pdf->Ln(4); //salto de linea
$pdf->Cell(0,0,'Este documento no es deducible fiscalmente',0,0,L);
$pdf->Cell(0,0,"Iva: "."$".number_format($ivaCargos[0],2),0,0,R);
}
*/
