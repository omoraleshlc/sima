...
$sSQL311= "Select  * From clientesInternos WHERE keyClientesInternos='".$_GET['keyClientesInternos']."' or folioVenta='".$_GET['folioVenta']."'";
...
$pdf->SetXY(2,41);
if($myrow311['ticket']>0){
$pdf->Cell(0,0,'http://www.hospitallacarlota.com  ticket: '.$myrow311['ticket'],0,0,L);
}elseif($_GET['cajero']){
...
//Usuario
$pdf->SetY(42);
$pdf->Cell(0,0,'Usuario Autorizo: '.$myrow311['usuario'],0,0,R);
...
if($myrow['iva']>0){
if($myrow['gpoProducto']!=NULL AND $myrow['naturaleza']=='C'){
$sumaIVA[0]+=$myrow['iva']*$myrow['cantidad'];
}elseif($myrow['gpoProducto']!=NULL and $myrow['naturaleza']=='A'){
$sumaIVADevs[0]+=$myrow['iva']*$myrow['cantidad'];
}    
}
...
if($myrow['precioVenta']>0){
if($myrow['gpoProducto']!=NULL AND $myrow['naturaleza']=='C'){
$importe2[0]+=$myrow['precioVenta']*$myrow['cantidad'];
}elseif($myrow['gpoProducto']!=NULL and $myrow['naturaleza']=='A'){
$importe2Devs[0]+=$myrow['precioVenta']*$myrow['cantidad'];  
}
}
...
if($myrow['gpoProducto']!=NULL AND $myrow['naturaleza']=='A'){
$signo='-';
} else {
$signo='';
}
...
//echo $importe2[0].' '.$importe2Devs[0];




$TOTAL=($importe2[0]-$importe2Devs[0])+($sumaIVA[0]-$sumaIVADevs[0]);
...
$pdf->Cell(0,0,"Subtotal: "."$ ".number_format($importe2[0]-$importe2Devs[0],2),0,0,R);




$pdf->SetY($Y+5);
$pdf->Cell(0,0,"Iva: "."$ ".number_format($sumaIVA[0]-$sumaIVADevs[0],2),0,0,R);
$pdf->SetY($Y+10);
$pdf->Cell(0,0,"Total: "."$ ".number_format(($importe2[0]-$importe2Devs[0])+($sumaIVA[0]-$sumaIVADevs[0]),2),0,0,R);
