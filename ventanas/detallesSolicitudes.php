...
$pdf->SetXY(2,25);
$pdf->Cell(0,0,"SOLICITUDES DIRECTAS",0,0,M);
$pdf->SetX(1);
...
$sSQLa= "
SELECT almacenSolicitante,almacenDestino FROM cargosCuentaPaciente
WHERE
...
while($myrowa = mysql_fetch_array($resulta)){
	...
	$checaModuloScript2a= "Select descripcion,almacenPadre from almacenes WHERE entidad='".$_GET['entidad']."' 
	and 
...
$sSQL40= "SELECT almacen,descripcion
FROM
almacenes
where 
entidad='".$_GET['entidad']."'
and
almacenPadre='".$resulScripModulo2a['almacenPadre']."'  ";
$result40=mysql_db_query($basedatos,$sSQL40);
$myrow40 = mysql_fetch_array($result40);
...
$debeCargo[0]+=$myrow['costoHospital']*$myrow['cantidad'];
}
...
$pdf->Cell(0,0,'$'.number_format($myrow['costoHospital']*$myrow['cantidad'],2),0,0,M);
$debeDevs[0]+=$myrow['costoHospital']*$myrow['cantidad'];
}
...
//*****************************SUMATORIAS A CENDIS************************
 ...
 $sSQLdes= "
     SELECT sum(costoHospital*cantidad) as cargosCendis 
...
naturaleza='C'
    ...
    (tipoPaciente='interno' or tipoPaciente='urgencias')
";
$resultdes=mysql_db_query($basedatos,$sSQLdes);
$myrowdes = mysql_fetch_array($resultdes);
 ...
 $sSQLde= "
     SELECT sum(costoHospital*cantidad) as devCendis 
FROM cargosCuentaPaciente
WHERE
...
$pdf->SetX('190');
$pdf->Cell(0,0,'$'.number_format($myrowdes['cargosCendis']-$myrowde['devCendis'],2),0,0,M);
...
$myrowde = mysql_fetch_array($resultde);
 ...
 $sSQLdeh= "
     SELECT sum(costoHospital*cantidad) as dev 
FROM cargosCuentaPaciente
WHERE
...
naturaleza='A'
    ...
    (tipoPaciente='interno' or tipoPaciente='urgencias')
";
$resultdeh=mysql_db_query($basedatos,$sSQLdeh);
$myrowdeh = mysql_fetch_array($resultdeh);
...
$pdf->Ln(4); //salto de linea */
$pdf->SetX('100');
$pdf->Cell(0,0,''.'$'.number_format($myrowde['cargos']-$myrowdeh['dev'],2),0,0,M);
