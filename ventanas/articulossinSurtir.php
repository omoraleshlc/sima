...
<?php 
if($_GET['keyCAP'] AND $_GET['inactiva']=='inactiva'){
 ...
 $q = "DELETE FROM cargosCuentaPaciente where keyCAP='".$_GET['keyCAP']."'";
		...
		echo 'Articulo eliminado';
                echo '<script>window.opener.document.forms["form1"].submit();</script>';
...
if($_POST['del']!=NULL){
 echo $q = "DELETE FROM cargosCuentaPaciente
...
<h1 align="center">Articulos Pendientes de Surtir</h1>
...
?>    
    ...
    <tr >
      ...
      <td  align="left">
	   ...
	   <div align="center" >	    
               <span >
  <?php if($myrow['fechaCargo']==''){ ?>                   
<a name="status<?php echo $f;?>" href="articulossinSurtir.php?keyCAP=<?php echo $myrow['keyCAP']; ?>&folioVenta=<?php echo $_GET['folioVenta']; ?>&amp;inactiva=<?php echo'inactiva'; ?>&amp;tipoAlmacen=<?php echo $_POST['tipoAlmacen']; ?>&amp;almacen=<?php echo $myrow['almacen'];?>&departamento=<?php echo $_POST['departamento'];?>#status<?php echo $f;?>"
onclick="if(confirm('Esta seguro que deseas eliminar este articulo?') == false){return false;}"   >
	 
       Eliminar
</a>
       <?php } else { ?>
       
           <img src="../imagenes/btns/lockbtn.png" alt="INACTIVO" width="20" height="20" border="0"  onclick="window.alert('Ya fue cargado este articulo, solo se puede devolver, no eliminar!');" />
       <?php } ?>
       </span></div>            
