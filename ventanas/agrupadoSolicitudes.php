...
$pdf->SetXY(2,5);
$pdf->Cell(0,0,'AGRUPADO DE POLIZA DE DIARIO',0,0,M);
...
$pdf->SetXY(2,25);
$pdf->Cell(0,0,"SOLICITUDES DIRECTAS A CENDIS",0,0,M);
$pdf->SetX(1);
...
cargosCuentaPaciente.fechaCargo='".$fecha1."'
and
cargosCuentaPaciente.almacenSolicitante='".$myrowa['almacenSolicitante']."'
    and
cargosCuentaPaciente.almacenDestino='".$cendis."'
    and
...
$sSQLdh= "SELECT sum(costoHospital*cantidad) as dev 
FROM cargosCuentaPaciente
WHERE
...
$resultdh=mysql_db_query($basedatos,$sSQLdh);
$myrowdh = mysql_fetch_array($resultdh);
...
$pdf->SetX('190');
if($myrowdh['dev']>0){
$pdf->Cell(0,0,'$'.number_format($myrowdh['dev'],2),0,0,M);
...
$pdf->SetX('190');
$pdf->Cell(0,0,'$'.number_format($myrowde['cendis']-$myrowdeh['dev'],2),0,0,M);
$haber[0]=$myrowde['cendis']-$myrowdeh['dev'];
...
$resultdh=mysql_db_query($basedatos,$sSQLdh);
$myrowdh = mysql_fetch_array($resultdh);
$debe[0]=$myrowd['cargos']-$myrowdh['dev'];
//***********************************************
