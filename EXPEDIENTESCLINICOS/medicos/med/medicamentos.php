...
if($_GET['actualizar'] and $_GET['medicamentos']){
$uploaddir = 'images/';
...
if($myrow25['numeroE']){
 ...
 $agrega = "UPDATE dx 
set
medicamentos='".$_GET['medicamentos']."',banderaMedicamentos='si'
where
keyClientesInternos='".$_GET['keyClientesInternos']."'
";
...
(medicamentos,entidad,keyClientesInternos,fecha,hora,usuario,medico,numeroE,nCuenta,numeroExpediente,seguro,banderaMedicamentos) values('".nl2br($_GET['medicamentos'])."','".$entidad."','".$_GET['keyClientesInternos']."','".$fecha1."','".$hora1."','".$usuario."','".$MEDICO."','".$numeroE."','".$nCuenta."','".$numeroE."','".$seguro."','si')";
mysql_db_query($basedatos,$agrega);
echo mysql_error();
...
?></p>
  <form id="form1" name="form1" method="GET" action="" onSubmit="return valida(this);" >
    ...
    <table width="591" border="0" align="center" class="style7">
      <tr>
        <td height="14" colspan="2" bgcolor="#660066"><div align="center"><span class="style13">MEDICAMENTOS</span></div></td>
      </tr>
      <tr>
        <td width="156" height="33" bgcolor="#660066">&nbsp;</td>
        <td width="514"><label>
<textarea name="medicamentos" cols="80" rows="10" wrap="virtual" id="medicamentos"><?php echo $myrow25['medicamentos'];?></textarea>
        </label></td>
      </tr>
      <tr>
        <td height="33">&nbsp;</td>
